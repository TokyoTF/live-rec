...
export default class Sites {
  ...
  // }
    ...
    if (recFormat === 'webm') {
      Logger.info(`WebM format incompatible with HLS fMP4 streams, using mkv instead`)
      recFormat = 'mkv'
    }
  ...
  recording: false,
    ...
    const handleEnd = () => {
      try {
        const exists = existsSync(outputPath)
        const size = exists ? statSync(outputPath).size : 0
        Logger.info(`ffmpeg end (${info.nametag}): output=${outputPath} exists=${exists} size=${size}`)
      } catch {}
      const r = Sites.#recordings.find(n => n.nametag === info.nametag && n.provider === info.provider)
