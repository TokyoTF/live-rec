...
export default class Sites {
  ...
  async recInit(info) {
    ...
    //This is shit code, remember remove this shit
    const resListinfo = info.resolutions && info.resolutions.reverse()
    const filterRes = info.resolutions && resListinfo?.findIndex(r => r.url === info.selresolution)
    const selRes = info.resolutions ? filterRes == -1 ? resListinfo.length > 0 ? resListinfo.length - 1 : 0 : filterRes : ''

    const onlyCR = info.provider == 'chaturbate' ? ['-map', '0:v:'+selRes, '-map', '0:a:0'] : ['-map', '0:v:0', '-map', '0:a:0']
  ...
  async rec(nametag, type, url, status, provider, dateformat,resolutions,selresolution, ffmpegparams) {
    try {
      let recording = Sites.#recordings.find(r => r.nametag === nametag && r.provider === provider)

      if (!recording) {
        if (type === 'startRec') {
          await this.recInit({ nametag, provider, dateformat,resolutions, selresolution, ffmpegparams })
          return this.rec(nametag, type, url, status, provider, dateformat, null, null, ffmpegparams)
        }
        return false
      }
  ...
  modifyjson(args) {
    try {
      ...
      } else if (args.raw.name === 'reclistupdate') {
        const idx = config.reclist.findIndex(r =>
          r.nametag === args.raw.value.nametag && r.provider === args.raw.value.provider
        )
        if (idx !== -1) config.reclist[idx] = args.raw.value
      } else {
