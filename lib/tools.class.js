...
export default class Sites {
  ...
  async rec(nametag, type, url, status, provider, dateformat,resolutions,selresolution, ffmpegparams, referer = '', keepAlive = true) {
    try {
      ...
      if (type === 'startRec' && !recording.recording && url && status === 'online') {
        ...
        await this.recInit({ nametag, provider, dateformat,resolutions,selresolution, ffmpegparams, referer, keepAlive, url })
  ...
  static #attemptReconnect(nametag, provider) {
    ...
    if (!r || !r.keepAlive || r.retryCount >= r.retryMax) {
      if (r) {
        Logger.stop(`Recording finished: ${nametag}`)
        r.recording = false
        r.keepAlive = false
      }
      return
    ...
    r.retryCount++
    r.recording = false
    Logger.info(`Attempting reconnect ${r.retryCount}/${r.retryMax} for ${nametag}...`)
