...
export default class Sites {
  ...
  // }
    ...
    let recFormat = (config.recformat || 'mkv').trim()
  ...
  recording: false,
    ...
    const handleError = (err) => {
      ...
      const isRecoverable = Sites.#isRecoverableError(err.message)
      if (!isRecoverable && Sites.#onError) {
        Sites.#onError({ nametag: info.nametag, provider: info.provider, error: this.errorFormat(info.nametag, info.provider, err.message) })
      }
      if (isUserStop || !isRecoverable) {
        if (r && !r.paused) {
          r.recording = false
          r.keepAlive = false
          if (!isUserStop && r.outputPath && !r.files.includes(r.outputPath)) {
            r.files.push(r.outputPath)
          }
          if (!isUserStop && r.files.length > 1) {
            const errCfg = this.loadjson()
            if (errCfg.concatonresume !== false) {
              Sites.#concatFiles(info.nametag, info.provider, [...r.files], r.outputPath)
            }
          }
        }
        return
    ...
    const handleEnd = () => {
      ...
      if (r && (r.paused || !r.keepAlive)) {
        ...
        r.recording = false
        if (outputPath && !r.files.includes(outputPath)) {
          r.files.push(outputPath)
        }
        if (r.files.length > 1) {
          const endCfg = this.loadjson()
          if (endCfg.concatonresume !== false) {
            Sites.#concatFiles(info.nametag, info.provider, [...r.files], r.outputPath)
          }
        }
        const mainWin = BrowserWindow.getAllWindows()[0]
        if (mainWin && !mainWin.isDestroyed()) {
          mainWin.webContents.send('rec:live:status', {
            nametag: info.nametag,
            provider: info.provider,
            status: false,
            paused: false,
            ...
            codec: null,
            stats: null,
            timeRec: this.getRecTime(r),
            outputPath: r.outputPath || null,
            url: r.url || null,
            selresolution: r.selresolution || null,
            provider_: r.provider || info.provider,
            files: r.files || [],
            fileSize: this.#getFilesSize([...(r.files || []), r.outputPath].filter(Boolean)),
    ...
    if (index !== -1) {
      ...
      recObj.files = info.files !== undefined ? recObj.files : (existing.files.length > 0 ? existing.files : (recObj.outputPath ? [recObj.outputPath] : []))
  ...
  recording.realtime = true
        ...
        const config = this.loadjson()
        const wasResumed = (recording.pausedAccumulator || 0) > 0
        if (filesToConcat.length > 1 && (config.concatonresume !== false || !wasResumed)) {
        ...
        if (status === 'offline') {
          ...
          const finalOutput = recording.outputPath
          const wasResumed = (recording.pausedAccumulator || 0) > 0
          const offCfg = this.loadjson()
          recording.startTime = null
          recording.files = []
          if (filesToConcat.length > 1 && (offCfg.concatonresume !== false || !wasResumed)) {
            Sites.#concatFiles(nametag, provider, filesToConcat, finalOutput)
          }
          try {
            const calPath = path.join(Sites.#FolderMain, 'calendar.json')
            if (existsSync(calPath)) {
              const calData = JSON.parse(readFileSync(calPath, 'utf-8'))
              const openEv = (calData.onlineEvents || []).find(
                e => e.nametag === nametag && e.provider === provider && !e.offlineAt
              )
              if (openEv) {
                openEv.offlineAt = Date.now()
                writeFileSync(calPath, JSON.stringify(calData, null, 2))
                const mainWin = BrowserWindow.getAllWindows()[0]
                if (mainWin && !mainWin.isDestroyed()) {
                  mainWin.webContents.send('calendar:load', calData)
                }
              }
            }
          } catch (e) {}
          ...
          if (mainWin && !mainWin.isDestroyed()) {
            mainWin.webContents.send('rec:live:status', {
              ...
              selresolution: recording.selresolution || null,
              provider_: recording.provider || provider,
          ...
          if (config.pauseforprivate !== false) {
            ...
            const filesToStop = [...recording.files]
            recording.startTime = null
            recording.files = []
            try { recording.objectFFmpeg.kill() } catch (e) { /* already dead */ }
            if (filesToStop.length > 1) {
              const privCfg = this.loadjson()
              if (privCfg.concatonresume !== false) {
                Sites.#concatFiles(nametag, provider, filesToStop, recording.outputPath)
              }
            }
          }
  ...
  removeRec(nametag, provider) {
    ...
    if (index !== -1) {
      ...
      if (filesToConcat.length > 1) {
        const rmCfg = this.loadjson()
        if (rmCfg.concatonresume !== false) {
          Sites.#concatFiles(nametag, provider, filesToConcat, finalOutput)
        }
      }
    }
  ...
  static #concatFiles(nametag, provider, files, finalOutput) {
    ...
    return new Promise((resolve) => {
      ...
      if (config.concatonresume === false) {
        resolve()
        return
      }
      const ffmpegPath = config.ffmpegselect
      const mkvmergePath = config.mkvmergepath || 'mkvmerge'

      const existingFiles = files.filter(f => existsSync(f)).map(f => f.trim())
      ...
      const tempOutput = finalOutput.slice(0, -ext.length) + '.tmp' + ext
      const isMKV = ext.toLowerCase() === '.mkv'

      const mainWin = BrowserWindow.getAllWindows()[0]
      const notifyStart = () => {
        if (mainWin && !mainWin.isDestroyed()) {
          mainWin.webContents.send('rec:concat', { nametag, provider, status: 'start', fileCount: existingFiles.length })
        }
      }
      const notifyDone = () => {
        if (mainWin && !mainWin.isDestroyed()) {
          mainWin.webContents.send('rec:concat', { nametag, provider, status: 'done' })
        }
      }
      ...
      if (isMKV && config.mkvmergenable !== false) {
        const args = ['-o', tempOutput, ...existingFiles]
        Logger.info(`Using mkvmerge: ${mkvmergePath} ${args.join(' ')}`)
        setTimeout(() => {
          execFile(mkvmergePath, args, { windowsHide: true }, (err) => {
            if (err) {
              Logger.error(`mkvmerge concat failed for ${nametag}: ${err.message}, falling back to ffmpeg`)
              Sites.#concatWithFfmpeg(ffmpegPath, existingFiles, tempOutput, finalOutput, nametag, notifyDone, cleanupSources, resolve)
            } else {
              Logger.info(`mkvmerge concat done: ${finalOutput}`)
              notifyDone()
  ...
  static #concatWithFfmpeg(ffmpegPath, existingFiles, tempOutput, finalOutput, nametag, notifyDone, cleanupSources, resolve) {
    const ext = path.extname(finalOutput)
    const listPath = finalOutput + '.concat.txt'
    const listContent = existingFiles.map(f => `file '${f.replace(/\\/g, '/').replace(/'/g, "'\\''")}'`).join('\n')
    writeFileSync(listPath, listContent)

    Logger.info(`Using ffmpeg concat: ${existingFiles.length} files`)
    ...
    setTimeout(() => {
      ffmpeg()
        ...
        .output(tempOutput)
        .on('end', () => {
          Logger.info(`ffmpeg concat done: ${finalOutput}`)
          notifyDone()
          try { renameSync(tempOutput, finalOutput) } catch {}
          cleanupSources()
        ...
        .on('error', (err) => {
          Logger.error(`ffmpeg concat failed for ${nametag}: ${err.message}`)
          notifyDone()
  ...
  static #attemptReconnect(nametag, provider) {
    ...
    if (!r || r.paused || !r.keepAlive || r.retryCount >= r.retryMax) {
      if (r && !r.paused) {
        ...
        r.recording = false
        r.keepAlive = false
        if (r.outputPath && !r.files.includes(r.outputPath)) {
          r.files.push(r.outputPath)
        }
        if (r.files.length > 1) {
          const reCfg = new Sites().loadjson()
          const wasResumed = (r.pausedAccumulator || 0) > 0
          if (reCfg.concatonresume !== false || !wasResumed) {
            Sites.#concatFiles(nametag, provider, [...r.files], r.outputPath)
          }
        }
    ...
    if (mainWin && !mainWin.isDestroyed()) {
      mainWin.webContents.send('rec:live:status', {
        ...
        stats: null,
        timeRec: (() => {
          if (!r || (!r.recording && !r.paused) || !r.startTime) return 0
          let time = Date.now() - r.startTime - (r.pausedAccumulator || 0)
          if (r.pausedAt) time -= (Date.now() - r.pausedAt)
          return Math.max(0, time)
        })(),
        outputPath: r.outputPath || null,
        url: r.url || null,
        selresolution: r.selresolution || null,
        provider_: r.provider || provider,
        files: r.files || [],
        fileSize: (() => {
          const files = [...(r.files || []), r.outputPath].filter(Boolean)
          let totalBytes = 0
          for (const f of files) {
            try { if (existsSync(f)) totalBytes += statSync(f).size } catch {}
          }
      ...
      try {
        ...
        if (recObj.url) {
          try {
            ...
            const failedRec = Sites.#recordings.find(n => n.nametag === nametag && n.provider === provider)
            if (failedRec) {
              failedRec.retryCount = savedRetryCount
              failedRec.keepAlive = savedKeepAlive
            }
          }
          const afterRec = Sites.#recordings.find(n => n.nametag === nametag && n.provider === provider)
          if (afterRec) {
            afterRec.keepAlive = savedKeepAlive
            if (savedFiles.length > 0 && (!afterRec.files || afterRec.files.length === 0)) {
              afterRec.files = savedFiles
            }
          }
        }
      } catch (err) {
