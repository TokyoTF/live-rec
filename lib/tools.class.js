  ...
  async rec(nametag, type, url, status, provider, dateformat) {
    try {
      ...
      if (
        ...
        recordings[selectedIndex].objectFFmpeg
          .input(url)
          .addOptions(['-map', '0:v:0', '-map', '0:a:0'])
        recordings[selectedIndex].objectFFmpeg.on('start', function () {
          console.log(`Start rec/${nametag}-${provider}`)
          recordings[selectedIndex].realtime = true
        })
        ...
        recordings[selectedIndex].recording = false
        recordings[selectedIndex].objectFFmpeg.kill()
      } else if (type == 'realtimeRec' && recordings[selectedIndex]) {
        console.log('checkRealTime')
      }
      return recordings[selectedIndex] ? recordings[selectedIndex] : false
    } catch (error) {
      let selectedIndex = recordings.findIndex((n) =>
        n.nametag == nametag && n.provider == provider ? n : null
      )
